...
#'          
RCAPlot <- function(rca_obj,point_cex=1,cluster_color_labels=NULL, fsize=1){
...
#  tiff("RCAplot_PCA_RCA_clusters.tiff")
  pdf("RCAplot_PCA_RCA_clusters.pdf")
  ...
  plot(cell_projection[,1],cell_projection[,2],
       ...
       font.axis = 1, font.lab = 1, font.main = 1,cex.axis=2
      );
  dev.off();
...
#  tiff("RCAplot_PCA_external_labels.tiff")
  pdf("RCAplot_PCA_external_labels.pdf")
  main_name = "PCA of cell clusters in RCA space";
  #par(mar=c(1,1,1,1)*7)  
  plot(cell_projection[,1],cell_projection[,2],
       type="p",pch = pch_to_use,
...
#  tiff("RCAplot_heatmap_RCA_clusters.tiff",width = 3000, height = 3000, units = "px", pointsize = 15);
  pdf("RCAplot_heatmap_RCA_clusters.pdf");
#  setEPS()
#  postscript("RCAplot_heatmap_RCA_clusters2.eps");

  color_scheme =     colorRampPalette(c("#00007F", "blue", "#007FFF", "cyan",
                       "#7FFF7F", "yellow", "#FF7F00", "red", "#7F0000"))(100);

#  color_scheme =     colorRampPalette(c("#00007F", "blue","yellow", "#FF7F00"))(100);
#  color_scheme =     colorRampPalette(c("blue","yellow"))(100);
					   
  heatmap.2(as.matrix(rca_obj$fpkm_for_clust),col=color_scheme,
          Colv=as.dendrogram(rca_obj$cellTree),
          ColSideColors=c,
           scale="none", # margins=c(5,50),
	   margins=c(5,16),
           trace="none",
           key = FALSE,
           keysize = 0.5,
           cexCol = 1,cexRow =fsize,
           labCol = "",
		   dendrogram="column"
           )  
  dev.off();
  ...
  if (!is.null(cluster_color_labels)){  
#  tiff("RCAplot_heatmap_external_labels.tiff",width = 3000, height = 3000, units = "px", pointsize = 15);
  pdf("RCAplot_heatmap_external_labels2.pdf",width = 3000, height = 3000)#,width = 3000, height = 3000, units = "px", pointsize = 15);
